...
#define RAYLEIGH_AMOUNT 1.5// Strength of atmospheric scattering (atmospheric density). [0.5 1.0 1.5 2.0]
...
#define WATER_CAUSTICS // Water Caustics.
#define STAR // Star.
#define TORCHLIGHT_BRIGHTNESS 0.75 // How bright is light from torches, fire, etc. [0.25 0.5 0.75 1.0 1.5 2.0]
...
void WaterFog(inout vec3 color, in SurfaceStruct surface, in MCLightmapStruct mcLightmap)
{
	...
	if (surface.mask.water > 0.5 || isEyeInWater > 0)
	{
		...
		vec3 waterFogColor = vec3(0.1, 2.4, 1.5) * 0.75; //clear water
...
void IceFog(inout vec3 color, in SurfaceStruct surface, in MCLightmapStruct mcLightmap)
{
	...
	if (surface.mask.ice > 0.5)
	{
		...
		// vec3 waterFogColor = vec3(0.2, 0.95, 0.4) * 5.0; //green water
		// vec3 waterFogColor = vec3(0.2, 0.95, 1.0) * 1.0; //clear water
		vec3 waterFogColor = vec3(1.2, 0.05, 0.55) * 0.75; //clear water
			  waterFogColor *= 0.01 * dot(vec3(0.33333), colorSunlight);
			  waterFogColor *= (1.0 - rainStrength * 0.95);
...
finalComposite.rgb = mix(finalComposite.rgb , vec3(0.8,0.8,0.9) * 0.01, stars * float(surface.mask.sky));
...
void main() {
	...
	//Apply lightmaps to albedo and generate final shaded surface
	vec3 finalComposite = final.sunlight 			* 0.8f 	* 1.5f * sunlightMult				//Add direct sunlight
