...
#define CINEMATIC_EFFECT // Cinematic Effect.
#define CE 0.05 // Adjust Cinematic Effect strip width. [0.03 0.05 0.07 0.9 0.11 0.13]
#define RAIN_LENS // Rain Lens.
...
void CalculateExposure(inout vec3 color) {
	...
	float exposure = pow(eyeBrightnessSmooth.y / 240.0f, 0.8f) * exposureMax + exposureMin;
...
void 	AddRainFogScatter(inout vec3 color, in BloomDataStruct bloomData)
{
    ...
    fogBlur*=vec3(0.5,0.5,0.5)*timeSunriseSunset +vec3(0.1,0.7,0.8)*timeMidnight +vec3(0.5,0.5,0.5)*rainStrength + vec3(0.5,0.5,0.5)*timeNoon;
	float linearDepth = GetDepthLinear(texcoord.st);

	float fogDensity = 0.007f * (rainStrength);
    fogDensity = fogDensity + 0.005f * timeSunriseSunset + 0.005f *timeMidnight+0.005*timeNoon;
...
void Cinematic_Effect(inout vec3 color) {
...
if (texcoord.t > 0.0 && texcoord.t < CE) {
color.rgb *= 0.0;
}
...
float gen = 1.0-fract((ftime+0.5)*0.5);
	...
	vec2 Fake_Refract_1 = vec2(sin(frameTimeCounter*1.7 + texcoord.x*50.0 + texcoord.y*25.0),cos(frameTimeCounter + texcoord.y*100.0 + texcoord.x*25.0)) ;
		...
		vec3 color = GetColorTexture(texcoord.st + fake_refract * 0.005 + 0.0045 * (rainlens + Fake_Refract_1*0.0045));	//Sample gcolor texture
			 color += rainlens*vec3(0.06,0.08,0.09)/119999*timeNoon;
			 color += rainlens*vec3(0.06,0.08,0.09)/239999*timeMidnight/33;
